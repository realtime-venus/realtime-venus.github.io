...
The closing gallery presents six supplied recordings in three groups: smart glasses (gaze-based museum guidance and menu recognition), smart rings (payment and capture), and smartphones (phone assistant and video verification). Each group contains two cards side by side on desktop and stacks on mobile. The phone videos retain their portrait format. Both ring videos and their posters use the supplied redacted September 21 versions; the phone videos preserve their complete recordings, original audio, and any existing redactions. Descriptive English labels identify the video content, without claiming that all examples use the Realtime-Venus model.
